...
echo "<td width=15%><b>$id $title:</b></td>";
echo "<td>$source</td>";
echo "<td>";

if ($filenametoshow == "jpg" || $filenametoshow == "gif" || $filenametoshow == "JPG" || $filenametoshow == "GIF" )
{
echo "<a href=/inthenews/$filename><img src=/inthenews/$filename width=150 border=0 alt=image></a><BR>";
...
echo " <a href=/inthenews/$filename>$filename</a><BR>";
echo "<a href=editfile.php?id=$id>edit file</a><BR>";
echo "<form method=post action=delete_download.php><input type=hidden name=id value=$id><input type=submit name=submit_delete value='delete file'></form><BR>";
}
...
echo "<a href=http://$webslink target=new>(check link)</a><BR><BR>";
	...
	echo "<textarea onClick='javascript: this.focus(); this.select();' rows='5' cols='20' readonly><img src=$websitefullurl/inthenews/$filename border=0 width=150 align=left alt=image></textarea>";
		}
...
echo "<a href=$websitefullurl/inthenews/$filename target=new>(check link)</a><BR><BR>";


if ($filenametoshow == "jpg" || $filenametoshow == "gif" || $filenametoshow == "JPG" || $filenametoshow == "GIF" )
{
echo "<span class=linkbrown>COPY and PASTE</span><BR>\n";
echo "<textarea onClick='javascript: this.focus(); this.select();' rows='5' cols='20' readonly><a href=$websitefullurl/inthenews/$filename><img src=$websitefullurl/inthenews/$filename border=0 width=150 align=left alt=image></a></textarea>";
}

else {
echo "<span class=linkbrown>COPY and PASTE</span><BR>\n";
echo "<textarea onClick='javascript: this.focus(); this.select();' rows='5' cols='20' readonly><a href=$websitefullurl/pages/main.php?pageid=63&pagecategory=5#$id target=new>$title</a></textarea>";
}
...
echo "<td><form method=post action=delete_inthenews.php><input type=hidden name=id value=$id><input type=submit name=submit_delete value=delete></form></td>";
echo "</tr>";}$e++; ?>
